<div
    class="doctor-luxury-prescriptions relative flex h-[100dvh] flex-col overflow-hidden bg-slate-50 lg:hidden"
    data-test="doctor-luxury-prescriptions"
>
    <header class="shrink-0 bg-gradient-to-b from-white to-slate-50 px-5 pb-4 pt-[max(2.25rem,env(safe-area-inset-top))]">
        <div class="flex items-center gap-3">
            ...
            <h1 class="min-w-0 flex-1 text-xl font-bold tracking-tight text-slate-900">{{ __('doctor.prescriptions_list.title') }}</h1>
        </div>
    </header>
    ...
    <main class="doctor-luxury-scroll mx-auto flex min-h-0 w-full max-w-md flex-1 flex-col gap-3 overflow-y-auto overscroll-contain px-5 pb-[calc(5.5rem+env(safe-area-inset-bottom))] pt-1">
        @forelse ($this->prescriptions as $appointment)
            <a
                ...
                wire:navigate
                wire:key="doctor-rx-{{ $appointment->id }}"
                ...
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        @if ($appointment->appointment_number)
                            <p class="mt-0.5 text-[0.625rem] font-bold uppercase tracking-wide text-slate-400">{{ $appointment->appointment_number }}</p>
                        @endif
                    ...
                    <span class="shrink-0 rounded-full px-2.5 py-1 text-[0.625rem] font-bold {{ $this->statusClasses($appointment) }}">
        ...
        @empty
            <div class="rounded-3xl border border-slate-100 bg-white px-6 py-14 text-center shadow-[0_4px_20px_-2px_rgba(0,0,0,0.03)]">
                <div class="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-slate-100 text-slate-400">
                    <flux:icon name="clipboard-document-list" variant="outline" class="size-7" />
                </div>
                <p class="text-sm leading-relaxed text-slate-500">{{ __('doctor.prescriptions_list.empty') }}</p>
            </div>
        @endforelse
    </main>
</div>
