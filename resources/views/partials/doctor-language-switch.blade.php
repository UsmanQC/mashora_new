...
<a
    href="{{ route('doctor.locale', ['locale' => $alternateLocale]) }}"
    wire:navigate="false"
    class="{{ $singleLinkClass }}"
    role="button"
    aria-label="{{ __('doctor.language.aria') }}"
    @if ($variant === 'chrome' || $variant === 'header' || $variant === 'guest') data-test="doctor-navbar-language-switch" @endif
    @if ($variant === 'menu') data-test="doctor-account-menu-language-switch" @endif
>
    {{ $alternateLabel }}
</a>
