...
<div class="patient-luxury-home relative min-h-[calc(100svh-7rem)] overflow-hidden bg-slate-50 pb-8 sm:hidden" data-test="patient-luxury-home">
    <header class="bg-gradient-to-b from-white to-slate-50 px-6 pb-6 pt-[max(3rem,env(safe-area-inset-top))]">
        <div class="flex items-center justify-between gap-3">
                    ...
                    <div class="relative shrink-0">
                        @if ($this->profilePhotoUrl !== null)
                            <img
                                ...
                                alt=""
                                class="size-12 rounded-full object-cover shadow-sm ring-2 ring-white"
                            />
                        @else
                            <flux:avatar :name="Auth::user()?->name ?? ''" circle class="size-12 ring-2 ring-white" />
                        @endif
                        <span class="absolute bottom-0 end-0 size-3.5 rounded-full border-2 border-white bg-[#10B981]" aria-hidden="true"></span>
                    </div>
                    <div class="min-w-0">
                        <p class="mb-0.5 text-xs font-medium text-slate-500">{{ $this->greetingLabel }}</p>
                        <h1 class="truncate text-lg font-bold tracking-tight text-slate-900">{{ Auth::user()?->name }}</h1>
                    </div>
                @else
                    <div class="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#10B981]/10 ring-2 ring-white">
                        @include('partials.patient-brand-logo', [
                            'svgClass' => 'h-6 w-auto max-w-[4.5rem] object-contain',
                            'onGreenChrome' => false,
                        ])
                    </div>
                    <div class="min-w-0">
                        <p class="mb-0.5 text-xs font-medium text-slate-500">{{ __('patient.home_luxury.guest_greeting') }}</p>
                        <h1 class="truncate text-lg font-bold tracking-tight text-slate-900">{{ __('patient.welcome_guest') }}</h1>
                    </div>
                @endif
            </div>
            ...
            @if ($isAuthenticated)
                <a
                    ...
                    wire:navigate
                    class="relative flex size-10 shrink-0 items-center justify-center rounded-full border border-slate-100 bg-white text-slate-600 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.03)] transition-colors hover:text-[#059669]"
                    aria-label="{{ __('patient.home_luxury.notifications_aria') }}"
                >
                    <flux:icon name="bell" variant="outline" class="size-5" />
                    @if ($this->unreadNotificationCount > 0)
                        <span class="absolute top-2.5 end-2.5 size-2 rounded-full border-2 border-white bg-red-500" aria-hidden="true"></span>
                    @endif
                </a>
            @else
                <a
                    href="{{ $phoneEntry }}"
                    wire:navigate
                    class="shrink-0 rounded-full bg-[#10B981] px-4 py-2 text-xs font-bold text-white shadow-sm transition-colors hover:bg-[#059669]"
                >
    ...
    <main class="mx-auto max-w-md space-y-8 px-6">
        <section class="rounded-3xl border border-slate-100 bg-white p-6 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.03)]">
            <div class="mb-5 flex items-end justify-between">
                <div>
                    <h2 class="mb-1 text-base font-bold text-slate-900">{{ __('patient.mood_section') }}</h2>
                    <p class="text-xs text-slate-500">{{ __('patient.home_luxury.mood_hint') }}</p>
                </div>
                <flux:icon name="chart-bar" variant="outline" class="size-5 text-[#10B981] opacity-50" />
            </div>

            <div class="patient-luxury-mood-row -mx-1 flex items-start justify-between gap-2 overflow-x-auto pb-1">
                        ...
                        <span class="patient-luxury-mood-icon">
                            @if ($mood['image_url'] !== null)
                                <img
                                    ...
                                    alt=""
                                    @class([
                                        'pointer-events-none size-8 object-contain',
                            ...
                            @else
                                <span @class([
                                    'pointer-events-none text-2xl leading-none',
        ...
        <section class="space-y-4">
            ...
            <a
                ...
                wire:navigate
                class="patient-luxury-action-card group relative overflow-hidden rounded-3xl bg-slate-900 p-5 shadow-[0_10px_40px_-10px_rgba(16,185,129,0.2)] transition-colors hover:bg-slate-800"
            >
            ...
            @if ($isAuthenticated && ($activeSession = $this->activeSessionAppointment))
                <a
                    ...
                    wire:navigate
                    class="patient-luxury-action-card group rounded-3xl border border-blue-100 bg-blue-50 p-5 transition-colors hover:bg-blue-100/50"
                >
            ...
            <a
                href="{{ $isAuthenticated ? $filterUrl : $phoneEntry }}"
                wire:navigate
                class="patient-luxury-action-card group rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.03)] transition-colors hover:border-emerald-200"
            >
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
            ...
            <a
                href="{{ $recordsUrl }}"
                wire:navigate
                class="patient-luxury-action-card group rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.03)] transition-colors hover:border-slate-300"
            >
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
