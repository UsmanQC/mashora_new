...
@php
    ...
    $isRtl = $locale === 'ar';
    $chevronIcon = $isRtl ? 'arrow-left' : 'arrow-right';
@endphp
...
<body class="relative">
    ...
    <header class="home-site-header fixed top-0 w-full z-50 glass-nav transition-all duration-300">
        <div class="home-site-header__inner max-w-[85rem] mx-auto flex items-center justify-between px-5 sm:px-6">
            ...
            <nav class="hidden md:flex items-center gap-12">
                <a href="#" class="text-sm font-medium text-primary-600 transition-colors">{{ __('marketing.nav.home') }}</a>
                <a href="{{ route('patient.schedule.specialists') }}" class="text-sm font-medium text-ink-muted hover:text-ink transition-colors">{{ __('marketing.nav.specialists') }}</a>
                <a href="#" class="text-sm font-medium text-ink-muted hover:text-ink transition-colors">{{ __('marketing.nav.assessments') }}</a>
                <a href="#" class="text-sm font-medium text-ink-muted hover:text-ink transition-colors">{{ __('marketing.nav.business') }}</a>
                <button
                    type="button"
                ...
                >
                    ...
                    {{ __('marketing.nav.ai_assistant') }}
                </button>
            </nav>

            <!-- Actions -->
            <div class="flex items-center gap-2 sm:gap-3">
                @include('partials.patient-language-switch', ['variant' => 'marketing'])
                <a
                ...
                <a href="{{ route('patient.phone') }}" class="hidden md:inline-flex text-sm font-medium text-ink-muted transition-colors hover:text-ink px-2">{{ __('marketing.login') }}</a>
            </div>
        </div>
    </header>
    ...
    <main class="home-main relative px-4 pb-16 sm:px-6 md:pb-24">
        <div class="max-w-[85rem] mx-auto">
            ...
            <div class="grid lg:grid-cols-12 gap-10 lg:gap-20 items-center">
                ...
                <div class="lg:col-span-5 text-start opacity-0 animate-fade-in-up">
    
                    <h1 class="text-4xl sm:text-5xl lg:text-[4.5rem] leading-[1.15] font-display font-bold text-ink mb-6 sm:mb-8 tracking-tight">
                        {{ __('marketing.hero.title_line1') }}<br>{{ __('marketing.hero.title_line2') }} <span class="text-primary">{{ __('marketing.hero.title_highlight') }}</span>
                    </h1>
    ...
    <section class="py-12 border-y border-slate-100 bg-surface-subtle">
        <div class="max-w-[85rem] mx-auto px-6">
            <p class="text-center text-sm font-medium text-ink-muted mb-10 tracking-wide">{{ __('marketing.trust.heading') }}</p>
            <div class="flex flex-wrap justify-center gap-16 md:gap-28 opacity-50 grayscale hover:grayscale-0 transition-all duration-700">
                <div class="flex items-center gap-3 font-display font-bold text-xl text-ink"><i data-lucide="heart-pulse" class="w-8 h-8 text-ink"></i> {{ __('marketing.trust.moh') }}</div>
                <div class="flex items-center gap-3 font-display font-bold text-xl text-ink"><i data-lucide="shield-check" class="w-8 h-8 text-ink"></i> {{ __('marketing.trust.national_center') }}</div>
                <div class="flex items-center gap-3 font-display font-bold text-xl text-ink"><i data-lucide="lock" class="w-8 h-8 text-ink"></i> {{ __('marketing.trust.hipaa') }}</div>
                <div class="flex items-center gap-3 font-display font-bold text-xl text-ink"><i data-lucide="cross" class="w-8 h-8 text-ink"></i> {{ __('marketing.trust.scfhs') }}</div>
            </div>
        </div>
    </section>
    ...
    <section class="py-40 bg-surface">
        <div class="max-w-[85rem] mx-auto px-6">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
                <div>
                    <h2 class="text-4xl lg:text-5xl font-display font-bold text-ink mb-6 tracking-tight">{{ __('marketing.specialists.heading') }}<br>{{ __('marketing.specialists.heading_break') }}</h2>
                    <p class="text-ink-muted text-lg max-w-xl leading-relaxed">{{ __('marketing.specialists.subtitle') }}</p>
                </div>
                <a href="{{ route('patient.schedule.specialists') }}" class="inline-flex items-center gap-2 text-primary font-bold hover:text-primary-700 transition-colors bg-primary-50 px-7 py-3.5 rounded-full">
                    {{ __('marketing.specialists.view_all') }} <i data-lucide="{{ $chevronIcon }}" class="w-4 h-4"></i>
                </a>
            </div>
                ...
                <div class="flex gap-8 overflow-x-auto overflow-y-hidden pb-16 pt-4 no-scrollbar">
                    ...
                    @empty
                        <div class="min-w-full rounded-[2rem] border border-dashed border-slate-200 bg-surface-subtle px-8 py-16 text-center">
                            <p class="text-lg font-bold text-ink mb-2">{{ __('marketing.specialists.empty_title') }}</p>
                            <p class="text-sm text-ink-muted mb-6">{{ __('marketing.specialists.empty_note') }}</p>
                            <a href="{{ route('patient.phone') }}" class="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-bold text-white transition hover:bg-primary-600">
                                {{ __('marketing.specialists.empty_cta') }}
                            </a>
                        </div>
                    @endforelse
                </div>
    ...
    <section class="py-40 bg-surface-subtle overflow-hidden border-t border-slate-100">
        <div class="max-w-[85rem] mx-auto px-6">
            <div class="grid lg:grid-cols-2 gap-24 items-center">
                ...
                <!-- Left: Text & Steps -->
                <div>
                    <h2 class="text-4xl lg:text-5xl font-display font-bold text-ink mb-8 tracking-tight">{{ __('marketing.privacy.heading') }}<br>{{ __('marketing.privacy.heading_break') }}</h2>
                    <p class="text-ink-muted text-lg mb-14 leading-relaxed max-w-lg">{{ __('marketing.privacy.subtitle') }}</p>
                    ...
                    <div class="space-y-10">
                        <div class="flex gap-6 items-start">
                            ...
                            <div>
                                <h4 class="font-display font-bold text-ink text-xl mb-2">{{ __('marketing.privacy.encryption_title') }}</h4>
                                <p class="text-ink-muted text-sm leading-relaxed max-w-md">{{ __('marketing.privacy.encryption_note') }}</p>
                            </div>
                        </div>
                        <div class="flex gap-6 items-start">
                            <div class="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center flex-shrink-0 text-primary border border-slate-100">
                            ...
                            <div>
                                <h4 class="font-display font-bold text-ink text-xl mb-2">{{ __('marketing.privacy.anonymous_title') }}</h4>
                                <p class="text-ink-muted text-sm leading-relaxed max-w-md">{{ __('marketing.privacy.anonymous_note') }}</p>
                            </div>
                        </div>
                        <div class="flex gap-6 items-start">
                            <div class="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center flex-shrink-0 text-primary border border-slate-100">
                            ...
                            <div>
                                <h4 class="font-display font-bold text-ink text-xl mb-2">{{ __('marketing.privacy.flex_title') }}</h4>
                                <p class="text-ink-muted text-sm leading-relaxed max-w-md">{{ __('marketing.privacy.flex_note') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative h-[650px] bg-white rounded-[3rem] shadow-premium border border-slate-100 p-8 flex flex-col justify-between">
                    ...
                    <div class="flex justify-between items-center mb-8 px-2">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-bold text-ink tracking-wide">{{ __('marketing.privacy.session_live') }}</span>
                        </div>
                    ...
                    <div class="bg-surface-subtle border border-slate-100 p-5 rounded-2xl flex gap-4 items-start shadow-sm">
                        ...
                        <div class="pt-0.5">
                            <p class="text-sm text-ink font-bold mb-1.5">{{ __('marketing.privacy.ai_assistant') }}</p>
                            <p class="text-xs text-ink-muted leading-relaxed">{{ __('marketing.privacy.ai_note') }}</p>
                        </div>
                    </div>
        ...
        <div class="max-w-[85rem] mx-auto px-6">
            ...
            <div class="bg-primary rounded-[3rem] p-14 md:p-20 flex flex-col md:flex-row items-center justify-between gap-10 mb-28 relative overflow-hidden shadow-2xl">
                ...
                <div class="relative z-10 text-center md:text-start">
                    <h2 class="text-4xl md:text-5xl font-display font-bold text-white mb-5 tracking-tight">{{ __('marketing.cta.heading') }}</h2>
                    <p class="text-primary-50 text-lg font-medium max-w-xl">{{ __('marketing.cta.subtitle') }}</p>
                </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-16 border-b border-slate-800 pb-16 mb-12">
                <div class="col-span-2 md:col-span-1">
                    ...
                    <p class="text-sm text-slate-400 leading-relaxed max-w-xs">{{ __('marketing.footer.tagline') }}</p>
                </div>
                
                <div>
                    <h4 class="text-white font-bold mb-8 tracking-wide">{{ __('marketing.footer.platform') }}</h4>
                    <ul class="space-y-5 text-sm text-slate-400 font-medium">
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.about') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.browse') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.faq') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.contact') }}</a></li>
                    </ul>
                </div>

                <div>
                    <h4 class="text-white font-bold mb-8 tracking-wide">{{ __('marketing.footer.services') }}</h4>
                    <ul class="space-y-5 text-sm text-slate-400 font-medium">
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.instant') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.scheduled') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.assessments') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.enterprise') }}</a></li>
                    </ul>
                </div>

                <div>
                    <h4 class="text-white font-bold mb-8 tracking-wide">{{ __('marketing.footer.join') }}</h4>
                    <ul class="space-y-5 text-sm text-slate-400 font-medium">
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.join_specialist') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.therapist_guide') }}</a></li>
                        <li><a href="#" class="hover:text-primary-400 transition-colors">{{ __('marketing.footer.terms') }}</a></li>
                    </ul>
                </div>
            </div>

            <div class="flex flex-col md:flex-row justify-between items-center text-sm text-slate-500 font-medium">
                <p>{{ __('marketing.footer.copyright') }}</p>
                <div class="flex gap-8 mt-6 md:mt-0">
                    <a href="#" class="hover:text-white transition-colors">{{ __('marketing.footer.terms_link') }}</a>
                    <a href="#" class="hover:text-white transition-colors">{{ __('marketing.footer.privacy_link') }}</a>
                </div>
            </div>
        </div>
    </footer>
