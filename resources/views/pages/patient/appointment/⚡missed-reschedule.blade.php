...
new #[Layout('layouts::patient')] #[Title('Reschedule missed session')] class extends Component
{
    ...
    public function mount(Appointment $appointment): void
    {
        ...
        $service = app(PatientMissedAppointmentService::class);

        if (! $service->canResolve($appointment)) {
        ...
        $this->newDate = $today->format('Y-m-d');
    ...
    public function save(): void
    {
        ...
        try {
            app(PatientMissedAppointmentService::class)->reschedule(
                $user,
                $this->appointment,
                $this->newDate,
                $this->selectedTime,
...
<div class="mx-auto max-w-2xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    <div class="flex items-start justify-between gap-4">
        <div>
            <flux:heading size="xl" class="font-semibold text-[#10B981]">{{ __('patient.missed.reschedule_title') }}</flux:heading>
            <flux:text class="mt-1 text-zinc-600">{{ __('patient.missed.reschedule_subtitle') }}</flux:text>
        </div>
        <flux:button
            :href="route('patient.appointments', ['tab' => 'missed'])"
            wire:navigate
            variant="ghost"
            size="sm"
            icon="arrow-left"
            :aria-label="__('patient.missed.back')"
        />
    </div>
    ...
    <div class="rounded-2xl border border-zinc-200/90 bg-white p-5 shadow-sm sm:p-6">
        <form wire:submit="save" class="space-y-5">
            <flux:field>
                <flux:label>{{ __('patient.missed.date_label') }}</flux:label>
                <flux:input wire:model.live="newDate" type="date" min="{{ $this->minDate() }}" required />
                <flux:error name="newDate" />
            </flux:field>

            <flux:field>
                <flux:label>{{ __('patient.missed.time_label') }}</flux:label>
                ...
                @else
                    <div
                        ...
                        wire:key="missed-reschedule-slots-{{ $newDate }}"
