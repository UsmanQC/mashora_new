        ...
        try {
            ...
            if ($sms->isLive() && ! $ok) {
                ...
                $this->smsError = is_array($result)
                    ? (string) ($result['error'] ?? __('patient_auth.otp_send_failed'))
                    : __('patient_auth.otp_send_failed');
            ...
            report($e);
            $this->devOtpDisplay = null;
            $this->smsError = __('patient_auth.otp_send_failed');
        }
    }
...
<div class="flex min-h-0 w-full flex-col">
    ...
    @if (filled($smsError))
        <div class="mt-4 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-900" role="alert">
            {{ $smsError }}
        </div>
    @endif
